...
export default function TaskMonitor() {
  ...
  if (!task) {
    return (
      <div className="px-4 pt-6">
        <p className="text-sm text-del">{error || "task not found"}</p>
      </div>
    );
  }
  ...
  return (
    <div className="safe-top flex min-h-dvh flex-col px-4 pt-4">
      <Link to={`/projects/${task.project_id}`} className="mb-2 inline-block text-xs text-faint">
        ← Project
      </Link>
      ...
      <div className="mb-3">
            ...
            <span className="flex items-center gap-1.5 text-[11px] text-faint">
              <span
                className={`h-2 w-2 rounded-full ${
                  live ? "bg-add pulse-dot" : "bg-warn pulse-dot"
                }`}
              />
        ...
        <p className="mt-2 text-sm text-fg">{task.prompt}</p>
        <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[11px] text-faint">
      ...
      {error ? (
        <p className="mb-2 rounded-lg border border-red-950 bg-red-950/25 px-3 py-2 text-xs text-del">
          {error}
        </p>
      ) : null}
        ...
        {(["feed", "diff", "screenshots"] as Tab[]).map((t) => (
          <button
            ...
            className={`flex-1 rounded-lg py-2 text-xs font-medium capitalize transition-colors ${
              tab === t ? "bg-elevated text-fg" : "text-faint"
            }`}
          >
            {t}
          </button>
        ...
        <div className="flex-1 pb-4">
          {!diff || diff.files.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {diff.summary ? (
                <p className="text-xs text-faint">{diff.summary}</p>
              ) : null}
              {diff.files.map((f) => (
                <div
                  key={f.path}
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate font-mono text-xs text-fg">{f.path}</p>
                    <p className="font-mono text-[10px] uppercase tracking-wide text-faint">
                      {f.status}
                    </p>
                  </div>
                  <div className="flex shrink-0 gap-2 font-mono text-[11px]">
                    {f.additions > 0 ? <span className="text-add">+{f.additions}</span> : null}
                    {f.deletions > 0 ? <span className="text-del">-{f.deletions}</span> : null}
                  </div>
                </div>
              ))}
            </div>
      ...
      {!isTerminal ? (
        <div className="safe-bottom sticky bottom-0 -mx-4 mt-auto border-t border-line bg-canvas px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            />
            <button
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent text-canvas active:bg-white disabled:opacity-40"
            ...
            <button
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-red-950 bg-red-950/30 text-del active:bg-red-950/60 disabled:opacity-40"
              disabled={busy}
              onClick={() => void doStop()}
              aria-label="Stop"
            >
      ...
      {busy ? (
        <div className="fixed bottom-24 right-4 rounded-full border border-line bg-surface p-2 shadow-lg">
